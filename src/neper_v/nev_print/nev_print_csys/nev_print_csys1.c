...
#include"nev_print_csys_.h"

void
nev_print_csys (FILE * file, struct CSYSDATA CsysData, struct PRINT Print)
{
  int i;
  ...
  ut_array_1d_memcpy (CsysData.Coo, 3, O);

  for (i = 0; i < 3; i++)
    if (strlen (CsysData.Label[i]) > 0)
    {
      nev_print_arrow (file, O, g[i], edge_rad, edge_texture);
