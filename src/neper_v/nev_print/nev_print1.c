...
void
nev_print (char **expargv, int *pi, struct PRINT Print, struct TESS Tess,
           struct TESSDATA TessData, struct TESR Tesr,
           struct TESRDATA TesrData, struct NODES Nodes, struct MESH *Mesh,
           struct NODEDATA NodeData, struct MESHDATA *MeshData, int SQty,
           struct NODES *SNodes, struct MESH *SMesh2D,
           struct NODEDATA *SNodeData, struct MESHDATA **SMeshData,
           int **SElt2dElt3d, struct CSYSDATA CsysData, struct POINT Point,
           struct POINTDATA PointData)
{
  ...
  ut_list_break (Print.imagesize, NEUT_SEP_DEP, &vals, &qty);
  ...
  {
    nev_print_pov2png (Print.povray, filename, imagewidth, imageheight,
  ...
  for (i = 0; i <= 3; i++)
        ...
        && !strcmp (MeshData[i].ColDataType, "scal"))
    {
      ...
      if (ut_list_testelt (Print.format, NEUT_SEP_NODEP, "pov") == 0
          && ut_list_testelt (Print.format, NEUT_SEP_NODEP,
                              "pov:objects") == 0)
        remove (filename2);
    }
  ...
  if (TesrData.ColDataType != NULL && !strcmp (TesrData.ColDataType, "scal"))
  {
