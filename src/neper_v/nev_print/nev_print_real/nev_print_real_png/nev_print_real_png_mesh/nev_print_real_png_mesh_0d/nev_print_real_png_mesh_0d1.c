...
#include"nev_print_real_png_mesh_0d_.h"

void
nev_print_real_png_mesh_0d (FILE * file, struct PRINT Print, struct MESH *Mesh,
  ...
  if (MeshData[0][0].Qty > 0)
  {
    ...
    for (i = 1; i <= Mesh[0].EltQty; i++)
      if (Print.showelt0d[i] == 1)
      {
        ...
        if (print == 1)
        {
          nev_print_real_png_sphere (file, NodeData.Coo[Mesh[0].EltNodes[i][0]],
