...
#include"nev_print_real_png_mesh_nodes_.h"

void
nev_print_real_png_mesh_nodes (FILE * file, struct PRINT Print, struct NODES Nodes,
                      struct MESH *Mesh, struct DATA NodeData)
{
  ...
  for (i = 1; i <= Nodes.NodeQty; i++)
    if (Print.shownode[i])
    {
      ...
      nev_print_real_png_sphere (file, NodeData.Coo[i], NodeData.Rad[i], texture);
      printnode_qty++;
    }
