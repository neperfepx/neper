...
#include"nev_print_real_png_mesh_2d_.h"

void
nev_print_real_png_mesh_2d_print_real_faces (FILE * file, struct PRINT Print,
                               ...
                               struct DATA **MeshData)
{
  ...
  if (!strncmp (coldatatype, "elt", 3))
  {
    ...
    nev_print_real_png_mesh2d (file, N, M2D, Print.showelt2d, rgb, trs, "elt", Print);
  ...
  else
  {
    ...
    nev_print_real_png_mesh2d (file, N, M2D, Print.showelt2d, rgb, NULL, "node", Print);
...
void
nev_print_real_png_mesh_2d_print_real_edges (FILE * file, struct PRINT Print, struct NODES N,
                               struct MESH M1D, struct DATA **MeshData)
{
  ...
  for (i = 1; i <= M1D.EltQty; i++)
    nev_print_real_png_segment_wsph (file, N.NodeCoo[M1D.EltNodes[i][0]],
