...
void
nev_print_real_png_mesh_2d_print (FILE * file, struct PRINT Print, struct NODES N,
                         struct MESH M1D, struct MESH M2D,
                         struct DATA NodeData, struct DATA **MeshData)
{
  nev_print_real_png_mesh_2d_print_real_faces (file, Print, N, M2D, NodeData, MeshData);

  nev_print_real_png_mesh_2d_print_real_edges (file, Print, N, M1D, MeshData);

  return;
}

void
nev_print_real_png_mesh_2d_compress (struct MESH Mesh2D, int *showelt,
                            struct MESH *pM1D)
{
