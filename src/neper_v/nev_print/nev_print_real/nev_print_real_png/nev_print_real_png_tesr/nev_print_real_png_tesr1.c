...
#include"nev_print_real_png_tesr_.h"

void
nev_print_real_png_tesr (FILE * file, struct PRINT Print, struct TESR Tesr,
                struct DATA* TesrData)
{
  ...
  neut_mesh_str (dim, Tesr.size, NULL, &Nodes, Mesh + dim, NULL);
...
#pragma omp parallel for
  ...
  if (dim == 2)
  {
    ...
    nev_print_real_png_mesh_3d (file, Print, Nodes, Mesh, NodeData, MeshData, &tmp);
