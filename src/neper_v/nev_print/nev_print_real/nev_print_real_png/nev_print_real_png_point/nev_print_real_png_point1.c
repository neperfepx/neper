...
#include"nev_print_real_png_point_.h"

void
nev_print_real_png_point (FILE * file, struct POINT Point, struct DATA PointData,
  ...
  for (i = 1; i <= Point.Qty; i++)
    if (show[i])
    {
      ...
      if ((!strcmp (PointData.RadDataType, "none")
        ...
        || !strcmp (PointData.RadDataType, "int")
        || !strcmp (PointData.RadDataType, "real"))
       && (!PointData.SymbolDataType || strcmp (PointData.SymbolDataType, "string")))
        nev_print_real_png_sphere (file, PointData.Coo[i], PointData.Rad[i], texture);
      ...
      if (PointData.SymbolDataType && !strcmp (PointData.SymbolDataType, "string"))
      {
        ...
        if (!strcmp (symbol, "sphere"))
          nev_print_real_png_sphere (file, PointData.Coo[i], params[0], texture);

        else if (!strcmp (symbol, "cube"))
          nev_print_real_png_cube (file, PointData.Coo[i], params, texture,
                              PointData.BRad, bordertexture);

        else if (!strcmp (symbol, "cylinder"))
          nev_print_real_png_cyl (file, PointData.Coo[i], params, texture);

        else if (!strcmp (symbol, "arrow"))
          nev_print_real_png_arr (file, PointData.Coo[i], params, texture);

        else if (!strcmp (symbol, "circle") || !strcmp (symbol, "disc"))
          nev_print_real_png_disc (file, PointData.Coo[i], params, texture);

        else if (!strcmp (symbol, "ellipsoid"))
          nev_print_real_png_ellipsoid (file, PointData.Coo[i], params, texture);

        else if (!strcmp (symbol, "torus"))
          nev_print_real_png_tor (file, PointData.Coo[i], params, texture);
