...
#include"nev_print_real_png_csys_.h"

void
nev_print_real_png_csys (FILE * file, struct DATA CsysData, struct PRINT Print, double **pos)
{
  int i;
  ...
  for (i = 0; i < 3; i++)
    if (strlen (CsysData.Label[i]) > 0)
    {
      ...
      nev_print_real_png_arr (file, O, data, edge_texture);
      ...
      fprintf (file, "  texture { pigment { rgbt <%f,%f,%f,0.> } finish {ambient %f diffuse %f reflection %f} }",
           CsysData.Col[1][0] / 255., CsysData.Col[1][1] / 255.,
           CsysData.Col[1][2] / 255., Print.lightambient, Print.lightdiffuse, Print.lightreflection);
      ...
      if (!pos)
        ...
        fprintf (file, "object { X%d translate <%f,%f,%f>}\n",
