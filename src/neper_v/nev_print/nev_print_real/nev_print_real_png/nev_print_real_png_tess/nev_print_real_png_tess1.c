...
#include"nev_print_real_png_tess_.h"

void
nev_print_real_png_tess (FILE * file, struct PRINT Print, struct TESS Tess,
                struct DATA *TessData)
{
  ...
  // Writing pov file --------------------------------------------------
  // nev_print_real_png_tess_header (file, TessData);
  ...
  if (Tess.Dim == 3)
  {
    ...
    Trs = TessData[3].Trs ? ut_array_1d_max (TessData[3].Trs + 1, TessData[3].Qty) : 0;
  ...
  if (Tess.Dim == 3)
    for (i = 0; i < polyfaceqty; i++)
    {
      ...
      nev_print_real_png_polygon (file, Tess.FaceEq[face], Tess.FaceVerQty[face], coo,
  ...
  for (i = 1; i <= facelist[0]; i++)
  {
    ...
    if (Print.showfaceinter == 1 && neut_tess_face_ff (Tess, face) > 0)
    {
      ...
      {
        if (Tess.FacePt[face] <= 0)
          nev_print_real_png_segment (file, p, coo[j], faceinter_rad,
                             "faceinter_texture");
  ...
  for (i = 1; i <= edgelist[0]; i++)
  {
    ...
    nev_print_real_png_segment_wsph (file, coo[0], coo[1], TessData[1].Rad[edge], texture);
  ...
  for (i = 1; i <= verlist[0]; i++)
  {
    ...
    nev_print_real_png_sphere (file, Tess.VerCoo[ver], TessData[0].Rad[ver], texture);
  }
  ...
  for (i = 1; i <= seedlist[0]; i++)
  {
    ...
    nev_print_real_png_sphere (file, Tess.SeedCoo[seed], TessData[4].Rad[seed], texture);
  }
  ...
  for (i = 1; i <= crystallist[0]; i++)
  {
    ...
    if (!strcmp (Tess.CellCrySym, "cubic"))
    {
      ...
      nev_print_real_png_cube (file, coo, rad, texture, TessData[5].BRad, bordertexture);
    }
    else if (!strcmp (Tess.CellCrySym, "hexagonal"))
    {
      ...
      nev_print_real_png_hcp (file, coo, rad, texture, TessData[5].BRad, bordertexture);
    }
    else
    {
      rad[0] = TessData[5].Rad[crystal];
      nev_print_real_png_sphere (file, coo, TessData[5].Rad[crystal], texture);
    }
