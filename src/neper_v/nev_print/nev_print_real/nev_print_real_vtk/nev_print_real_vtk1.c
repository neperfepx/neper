...
#include"nev_print_real_vtk_.h"

void
nev_print_real_vtk (struct IN_V In, char *basename,
               ...
               struct DATA *pNodeData, struct DATA **MeshData)
{
  ...
  if ((*pNodeData).CooData || (*pNodeData).ColData)
  {
    ...
    if ((*pNodeData).CooData)
      nev_print_real_vtk_coodata (file, *pNodeData);

    if ((*pNodeData).ColDataType)
      nev_print_real_vtk_coldata (file, *pNodeData);
   }
  ...
  if (MeshData[3][0].ColDataType)
  {
    ...
    nev_print_real_vtk_coldata (file, MeshData[3][0]);
  }
