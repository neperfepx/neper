...
void
nev_tessdata_fscanf_init (struct TESS Tess, char *entity_in, char *prop,
                          char *argument, int *pid, int *pentityqty,
...
void
nev_tessdata_fscanf_col (struct TESS Tess, struct TESSDATA *pTessData,
                         char *entity, int id, int entityqty, char *type,
                         char *value)
{
  int i, scale;
  ...
  if (!strcmp (type, "col"))
  {
    ...
    ut_array_2d_fnscanf_wcard (value, (*pTessData).ColData[id] + 1, entityqty,
                               3, "colour,size");
  }
  else if (!strcmp (type, "id"))
  {
    ...
    if (!value)
      for (i = 1; i <= entityqty; i++)
        (*pTessData).ColData[id][i][0] = (!strcmp (entity, "cell")
                                          && Tess.CellId) ? Tess.
          CellId[i] : i;
    else
      ut_array_2d_fnscanf_wcard (value, (*pTessData).ColData[id] + 1,
                                 entityqty, 1, "numeral");
  }
  else if (!strncmp (type, "scaleid", 7))
  {
    ...
    {
      (*pTessData).ColData[id][i][0] = (!strcmp (entity, "cell")
                                        && Tess.ScaleCellId && scale >= 0
                                        && scale <=
                                        Tess.ScaleQty) ? Tess.
        ScaleCellId[i][scale] : i;
    }
  }
...
void
nev_tessdata_fscanf_trs (struct TESSDATA *pTessData, int id, int entityqty,
                         char *type, char *value)
{
  ...
  if (!strcmp (type, "trs") || !strcmp (type, "scal"))
  {
    ...
    ut_array_2d_fnscanf_wcard (value, (*pTessData).trsdata[id] + 1, entityqty,
                               1, "numeral,size");
  }
  else
    abort ();
...
void
nev_tessdata_fscanf_rad (struct TESSDATA *pTessData, int id, int entityqty,
                         char *type, char *value)
{
  ...
  if (!strcmp (type, "rad") || !strcmp (type, "scal"))
  {
    ...
    ut_array_2d_fnscanf_wcard (value, (*pTessData).RadData[id] + 1, entityqty,
                               1, "numeral,size");
  }
  else
    abort ();
...
void
nev_tessdata_fscanf_colscheme (struct TESSDATA *pTessData, int id, char *type)
{
...
void
nev_tessdata_fscanf_scale (struct TESSDATA *pTessData, int id, char *type)
{
...
void
nev_tessdata_fscanf_scaletitle (struct TESSDATA *pTessData, int id,
                                char *type)
{
