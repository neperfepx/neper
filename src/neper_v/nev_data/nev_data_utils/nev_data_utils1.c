...
nev_data_string_entity_type (char *string, char *entity, char *type)
{
  ...
  else if (strncmp (stringcpy, "rptedge", 7) == 0)
  {
    ut_print_message (1, 3,
                      "'rpt' is deprecated and will be removed in future versions.  Use 'vox' instead.\n");
...
{
  int status = 0;
  if (strcmp (type, "scal") == 0 || strcmp (type, "int") == 0
      || strcmp (type, "rad") == 0 || strcmp (type, "edgerad") == 0
      || strcmp (type, "real") == 0)
    (*psize) = 1;
  else if (strncmp (type, "ori", 3) == 0 || strcmp (type, "edgecol") == 0
                      ...
                      char *scheme, int **Col, char **pscale)
{
  ...
  if (scheme == NULL)
    for (i = 1; i <= size; i++)
      ut_color_bar_val_color ("blue,cyan,yellow,red", ScaleBeg, ScaleEnd,
                              data[i][0], Col[i]);
  else
    for (i = 1; i <= size; i++)
                       ...
                       char **pvalue, char **pmod)
{
  ...
  if (!strcmp (*ptype, "col"))
    return;
  else if (!strcmp (*ptype, "e") || !strcmp (*ptype, "ek")
           || !strcmp (*ptype, "er") || !strcmp (*ptype, "R")
           || !strcmp (*ptype, "q") || !strcmp (*ptype, "m")
           || !strcmp (*ptype, "rtheta") || !strcmp (*ptype, "g"))
  {
...
void
nev_data_fscanf_ori_tesr (struct TESR Tesr, char *value, int qty,
