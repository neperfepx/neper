...
void
nev_meshdata_fscanf_elt (struct MESH Mesh, struct MESHDATA *pMD, char *type,
                         char *argument, int elset)
{
  int i;
  char *value = NULL;
  char **args = NULL;
  ...
  if (!strcmp (type, "col"))
  {
    ...
    nev_data_typearg_args ("col", argument, &(*pMD).ColDataType, &value,
                           NULL);
    ...
    if (!strcmp ((*pMD).ColDataType, "id"))
    {
      ...
      else
      {
        if (argqty == 1)
          ...
          ut_array_2d_fnscanf (args[1], (*pMD).ColData + 1, (*pMD).EltQty, 1);
      }
    }
    else if (!strcmp ((*pMD).ColDataType, "col"))
    {
      (*pMD).ColData = ut_alloc_2d ((*pMD).EltQty + 1, 3);
      ut_array_2d_fnscanf_wcard (value, (*pMD).ColData + 1, (*pMD).EltQty, 3,
                                 "colour,size");
    }
    else if (!strncmp ((*pMD).ColDataType, "ori", 3))
      nev_data_fscanf_ori (value, (*pMD).EltQty, NULL, &(*pMD).ColData,
                           &(*pMD).ColDataType);
    else if (!strcmp ((*pMD).ColDataType, "scal"))
    {
      (*pMD).ColData = ut_alloc_2d ((*pMD).EltQty + 1, 1);
      ut_array_2d_fnscanf_wcard (value, (*pMD).ColData + 1, (*pMD).EltQty, 1,
                                 "numeral,size");
    }
    else
      abort ();
  ...
  else if (!strcmp (type, "rad"))
  {
    ...
    ut_array_2d_fnscanf_wcard (argument, (*pMD).RadData + 1, (*pMD).EltQty, 1,
                               "numeral,size");
  }
...
nev_meshdata_fscanf_eltb (struct MESHDATA *pMD, char *type, char *argument)
{
  if (!strcmp (type, "col"))
  {
    ...
    ut_array_1d_int_fnscanf_wcard (argument, (*pMD).BCol, 3, "colour");
  }
  else if (!strcmp (type, "rad"))
    ut_array_1d_fnscanf_wcard (argument, &((*pMD).BRad), 1, "numeral");
  else
    ut_print_exprbug (type);
...
void
nev_meshdata_elset2elt (struct MESH Mesh, char *entity, char *type,
                        struct MESHDATA MeshDataSet, struct MESHDATA *pMD)
{
  ...
  else
  {
    ...
    {
      (*pMD).BCol = ut_alloc_1d_int (3);
      ut_array_1d_int_memcpy (MeshDataSet.BCol, 3, (*pMD).BCol);
    }
    else if (!strcmp (type, "rad"))
