...
ANNdist
annDist (			// interpoint squared distance
	  int dim, ANNpoint p, ANNpoint q)
{
  ...
  ANN_FLOP (3 * dim)		// performance counts
    ANN_PTS (1) ANN_COORD (dim) return dist;
}
...
ANNpoint
annAllocPt (int dim, ANNcoord c)	// allocate 1 point
{
...
ANNpointArray
annAllocPts (int n, int dim)	// allocate n pts in dim
{
...
void
annDeallocPt (ANNpoint & p)	// deallocate 1 point
{
  delete[]p;
  p = NULL;
}
...
void
annDeallocPts (ANNpointArray & pa)	// deallocate points
{
...
ANNpoint
annCopyPt (int dim, ANNpoint source)	// copy point
{
  ANNpoint p = new ANNcoord[dim];
  for (int i = 0; i < dim; i++)
    p[i] = source[i];
...
void
annAssignRect (int dim, ANNorthRect & dest, const ANNorthRect & source)
{
  for (int i = 0; i < dim; i++)
  {
...
ANNbool
ANNorthRect::inside (int dim, ANNpoint p)
{
  for (int i = 0; i < dim; i++)
  {
...
void
annError (const char *msg, ANNerr level)
{
  if (level == ANNabort)
  {
...
void
annMaxPtsVisit (		// set limit on max. pts to visit in search
