...
double
annAspectRatio (int dim,	// dimension
...
void
annEnclRect (ANNpointArray pa,	// point array
...
void
annEnclCube (			// compute smallest enclosing cube
	      ...
	      ANNorthRect & bnds)	// bounding cube (returned)
{
  ...
  {				// determine max side length
    ...
    if (len > max_len)
    {				// update max_len if longest
      max_len = len;
    }
  }
  for (d = 0; d < dim; d++)
  {				// grow sides to match max
    ANNcoord len = bnds.hi[d] - bnds.lo[d];
...
ANNdist
annBoxDistance (		// compute distance from point to box
...
ANNcoord
annSpread (			// compute point spread along dimension
	    ANNpointArray pa,	// point array
	    ANNidxArray pidx,	// point indices
	    int n,		// number of points
...
void
annMinMax (			// compute min and max coordinates along dim
	    ANNpointArray pa,	// point array
	    ANNidxArray pidx,	// point indices
	    int n,		// number of points
	    int d,		// dimension to check
...
int
annMaxSpread (			// compute dimension of max spread
	       ANNpointArray pa,	// point array
	       ANNidxArray pidx,	// point indices
	       int n,		// number of points
	       int dim)		// dimension of space
...
void
annMedianSplit (ANNpointArray pa,	// points to split
		ANNidxArray pidx,	// point indices
		int n,		// number of points
		...
		int n_lo)	// split into n_lo and n-n_lo
{
  ...
  while (l < r)
  {
    ...
    if (PA (i, d) > PA (r, d))	// make sure last > pivot
      PASWAP (i, r) PASWAP (l, i);	// move pivot to first position
...
void
annPlaneSplit (			// split points by a plane
...
void
annBoxSplit (			// split points by a box
	      ANNpointArray pa,	// points to split
	      ANNidxArray pidx,	// point indices
	      int n,		// number of points
	      int dim,		// dimension of space
...
int
annSplitBalance (		// determine balance factor of a split
		  ANNpointArray pa,	// points to split
		  ANNidxArray pidx,	// point indices
		  int n,	// number of points
		  int d,	// dimension along which to split
...
void
annBnds2Box (const ANNorthRect & bnd_box,	// enclosing box
	     int dim,		// dimension of space
