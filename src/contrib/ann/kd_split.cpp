...
void
kd_split (ANNpointArray pa,	// point array (permuted on return)
...
{
  if (bnds.lo == 0) {} // trick to disable warning
...
void
midpt_split (ANNpointArray pa,	// point array
...
void
sl_midpt_split (ANNpointArray pa,	// point array
		ANNidxArray pidx,	// point indices (permuted on return)
		const ANNorthRect & bnds,	// bounding rectangle for cell
		int n,		// number of points
		int dim,	// dimension of space
...
void
fair_split (ANNpointArray pa,	// point array
	    ANNidxArray pidx,	// point indices (permuted on return)
	    const ANNorthRect & bnds,	// bounding rectangle for cell
	    int n,		// number of points
	    int dim,		// dimension of space
	    ...
	    int &n_lo)		// num of points on low side (returned)
{
  ...
  {				// median cut preserves asp ratio
...
void
sl_fair_split (ANNpointArray pa,	// point array
	       ANNidxArray pidx,	// point indices (permuted on return)
	       const ANNorthRect & bnds,	// bounding rectangle for cell
	       int n,		// number of points
	       int dim,		// dimension of space
	       ...
	       int &n_lo)	// num of points on low side (returned)
{
  ...
  {
    if (max > lo_cut)
    {				// are any points above lo_cut?
      cut_val = lo_cut;		// cut at lo_cut
      annPlaneSplit (pa, pidx, n, cut_dim, cut_val, br1, br2);
      n_lo = br1;		// balance if there are ties
    }
    else
    {				// all points below lo_cut
  ...
  {
    if (min < hi_cut)
    {				// are any points below hi_cut?
      cut_val = hi_cut;		// cut at hi_cut
      annPlaneSplit (pa, pidx, n, cut_dim, cut_val, br1, br2);
      n_lo = br2;		// balance if there are ties
    }
    else
    {				// all points above hi_cut
