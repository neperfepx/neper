		...
		double **c)
{
  ...
  ut_array_2d_memcpy (c, a1, b2, ccpy);
...
// ut_mat_eigen provides the eigen values eval and eigen vectors evect
// of a matrix A of size n x n.  eval and evect must be preallocated.
// The eigen values are sorted in descending order, and the eigen
// vectors given accordingly.
// The eigen vector corresponding to eval[0]   is evect[0][0..n-1]
// The eigen vector corresponding to eval[1]   is evect[1][0..n-1]
// ...
// The eigen vector corresponding to eval[n-1] is evect[n-1][0..n-1]
void
ut_mat_eigen (int n, double **A, double *eval, double **evect)
{
  double *e = (double *) calloc (n, sizeof (double));
  double **evect2 = ut_alloc_2d (n, n);
  double *eval2 = ut_alloc_1d (n);

  ut_array_2d_memcpy (evect2, n, n, A);
  tred2 (n, evect2, eval2, e);
  tql2 (n, evect2, eval2, e);
  free (e);

  ut_mat_transpose (evect2, n, n, evect2);

  if (eval)
    ut_array_1d_memcpy (eval, n, eval2);
  if (evect)
    ut_array_2d_memcpy (evect, n, n, evect2);
