...
int
ut_sys_runwtime (char *exec, char *command, double t, struct timeval *pctrlc_t)
{
  int status;
  pid_t pid;
  ...
  value.it_value.tv_usec =
    (long int) ((double) (t - value.it_value.tv_sec) * 1000000);
...
int
ut_sys_isdir (const char *pathname, ...)
{
  struct stat sb;
  ...
  vsnprintf (tmp, sizeof (tmp), pathname, args);
  va_end (args);
...
char *
ut_sys_realpath (const char *pathname)
{
...
{
  ssize_t r;
  // see http://linux.die.net/man/2/readlink for how not to use a fixed bufsize
  int bufsize = 1024;
