          ...
          struct POINT Point)
{
  ...
  if (In.stnode)
  {
    ...
    nem_stat_nodes (file, In.stnode, Nodes, Mesh, Tess);
  ...
  for (dim = 0; dim <= 5; dim++)
    if (In.stmesh[dim])
    {
      ...
      {
        file = ut_file_open (name, "w");
        nem_stat_elts (file, "mesh", dim, realdim, In.stmesh[dim], Nodes, Mesh, MeshPara, Tess);
        ut_file_close (file, name, "w");
      }
  ...
  for (dim = 0; dim <= 5; dim++)
    if (In.stelt[dim])
    {
      ...
      {
        file = ut_file_open (name, "w");
        nem_stat_elts (file, "elt", dim, realdim, In.stelt[dim], Nodes, Mesh, MeshPara, Tess);
        ut_file_close (file, name, "w");
      }
