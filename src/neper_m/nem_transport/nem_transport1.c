...
void
nem_transport (struct IN_M In, struct TESS Tess, struct NODES RNodes,
               struct MESH *RMesh, struct NODES Nodes, struct MESH *Mesh)
{
  ...
  ut_list_break (In.transportstring, NEUT_SEP_NODEP, &transport,
                 &transportqty);

  for (i = 0; i < transportqty; i++)
  {
    ut_list_break (transport[i], NEUT_SEP_DEP, &val, &valqty);
    ...
    if (!strcmp (val[0], "elt"))
      nem_transport_elt (val[1], val[2], In.transporteltmethodstring, RNodes,
                         RMesh[dim], Nodes, Mesh[dim], &oldelt);
                   ...
                   struct MESH *RMesh, struct NODES Nodes, struct MESH *Mesh)
{
  ...
  ut_list_break (In.transportfepxstring, NEUT_SEP_NODEP, &transport,
                 &transportqty);
  if (transportqty != 2)
  ...
  nem_transport_elt ("real3", transport[0], In.transporteltmethodstring,
                     RNodes, RMesh[dim], Nodes, Mesh[dim], &oldelt);

  ut_print_message (0, 3, "Transporting `tau'...\n");

  nem_transport_elt ("real1", transport[1], In.transporteltmethodstring,
                     RNodes, RMesh[dim], Nodes, Mesh[dim], &oldelt);
