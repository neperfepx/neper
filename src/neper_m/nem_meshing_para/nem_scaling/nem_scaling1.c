		  ...
		  struct MESHPARA *pMeshPara)
{
  ...
  ut_string_separate ((*pMeshPara).clstring, NEUT_SEP_NODEP, &cllist, &clqty);
  ut_string_separate (cllist[0], NEUT_SEP_DEP, &list, &qty);
  ...
  else if (qty == 3)
  {
    ...
    if (input == 't')
      neut_tess_scale (pTess, (*pMeshPara).scale[0],
	...
	(pTess, *pRNodes, RMesh[0], RMesh[1], RMesh[2], RMesh[3]);
  }
  else
    abort ();
  ...
  for (i = 1; i < clqty; i++)
  {
    ...
    ut_string_separate (cllist[i], NEUT_SEP_DEP, &list, &qty);
		  ...
		  struct TESR *pTesr, struct NODES *pNodes)
{
  ...
  {
    if ((*pTess).PolyQty > 0)
      neut_tess_scale (pTess, 1 / MeshPara.scale[0],
