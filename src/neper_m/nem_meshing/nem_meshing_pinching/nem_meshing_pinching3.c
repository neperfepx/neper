                                    ...
                                    int **peltfacets, int *peltfacetqty)
{
  ...
  neut_mesh_elset_elt_insidefacets (Mesh[1], Mesh[2], elset, elt,
				    peltfacets, peltfacetqty);
				...
				int elt, int eltfacet, int *pnewnode)
{
  ...
  neut_mesh_nodes_elsets_comelts (Mesh[2], nodes, 2, &elset, 1, &comelts, &comeltqty);
  if (comeltqty != 2)
    ut_error_reportbug ();
  ...
  *pnewnode = (*pNodes).NodeQty;
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    ut_array_1d_int_findnreplace (Mesh[2].EltNodes[comelt], 3, nodes[0], *pnewnode);
    ut_array_1d_int_findnreplace (Mesh[2].EltNodes[newelt], 3, nodes[1], *pnewnode);
  }
