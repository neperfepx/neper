...
void
nem_meshing_1d_useg (double cl1, double cl2, double clt, double pl, int *pqty,
                     double **pcoo, double **pcl)
{
  ...
  if (cl1 > clt && cl1 < (1 + 1e-6) * clt)
    cl1 = clt;

  if (cl2 > clt && cl2 < (1 + 1e-6) * clt)
    cl2 = clt;
  ...
  if (reverse)
    nem_meshing_1d_useg_reverse (*pqty, *pcoo, *pcl);
