...
nem_meshing_3D_poly_algo (struct IN_M In, double cl, double mesh3dclreps,
                          ...
                          struct NODES Nodes, struct MESH *Mesh, int poly,
                          struct NODES *pN, struct MESH *pM, double *pmOsize,
                          double *pelapsed_t)
{
  int iter, status = -1;
  double rnd;
  ...
  for (iter = 0; iter < In.mesh3diter; iter++)
  {
    ...
    status =
      nem_mesh_3d_gmsh (Tess, poly, Nodes, Mesh, cl, mesh3dclreps, In.gmsh,
                        In.tmp, (*pMultim).algos[algo][0],
                        (*pMultim).algos[algo][1], rnd, *pallowed_t, pN, pM,
                        pmOsize, pctrlc_t, pelapsed_t);
  ...
  if (!status)
  {
    ...
    if (!strcmp (In.meshqualexpr, "Odis^0.8*Osize^0.2"))
      (*pMultim).mO[poly][algo] =
        pow ((*pMultim).mOdis[poly][algo],
             0.8) * pow ((*pMultim).mOsize[poly][algo], 0.2);
    else
    {
...
void
nem_meshing_3D_report_poly_algo (int poly, int a, struct NODES N,
                                 struct MESH M)
{
  int i;
  double rr, length;
