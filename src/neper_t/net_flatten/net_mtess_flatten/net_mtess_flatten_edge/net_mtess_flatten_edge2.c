                             ...
                             struct TESSE *TessE, struct FLATTEN *pFlatten)
{
  ...
  for (i = 0; i < CTessQty; i++)
  {
    ...
    for (j = 1; j <= Tess[id].EdgeQty; j++)
      ...
      {
        neut_tess_addedge (pFTess,
                           TessE[id].VerFVer[Tess[id].EdgeVerNb[j][0]],
                           TessE[id].VerFVer[Tess[id].EdgeVerNb[j][1]]);
        neut_flatten_addedge (pFlatten, dom);

        net_mtess_flatten_edge_domedge_linkedge ((*pFTess).EdgeQty, TessE, id,
                                                 j, 1);
      }
  }
                                ...
                                struct TESSE *TessE, struct FLATTEN *pFlatten)
{
  ...
  for (i = 1; i <= Tess[TessId].FaceQty; i++)
  {
    ...
    net_mtess_flatten_edge_domface_edges (MTess, Tess, TessId, i, &VerQty,
                                          &VerCoo, &TessNb, &TessQty, &EdgeNb,
                                          &EdgeQty, &EdgeVerNb, &EdgeVerQty);
    ...
    for (t = 0; t < TessQty; t++)
      for (j = 0; j < EdgeQty[t]; j++)
        ut_array_1d_int_addval (EdgeVerNb[t][j], EdgeVerQty[t][j], VerQty_ini,
                                EdgeVerNb[t][j]);
    ...
    for (t = 0; t < TessQty; t++)
      for (j = 0; j < EdgeQty[t]; j++)
        if (EdgeVerQty[t][j] == 0)
          ...
          net_mtess_flatten_edge_domface_addedge_ivers (pFTess,
                                                        Tess[TessNb[t]],
                                                        &(TessE[TessNb[t]]),
                                                        EdgeNb[t][j],
                                                        EdgeVerNb[t][j],
    ...
    for (t = 0; t < TessQty; t++)
      for (j = 0; j < EdgeQty[t]; j++)
        ut_free_1d_int (&EdgeVerNb[t][j]);
    ut_free_1d_ppint (&EdgeVerNb);

    ut_free_2d_int (&EdgeVerQty, TessQty);
...
void
net_mtess_flatten_edge_init_fedgetessedge (struct TESS *Tess, int *CTessIds,
                                           int CTessQty, struct TESS *pFTess,
                                           struct TESSE *TessE,
