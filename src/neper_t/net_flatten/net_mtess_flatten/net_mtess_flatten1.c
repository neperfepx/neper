...
void
net_mtess_flatten (struct MTESS MTess, struct TESS *Tess, int TessId, struct TESS *pFTess)
{
  int i;
  ...
  neut_mtess_tess_poly_childtesses (MTess, Tess, TessId, &CTessIds, &CTessQty);
  ...
  net_mtess_flatten_ver (MTess, Tess, TessId, CTessIds, CTessQty, &FTess, TessE, &Flatten);

  net_mtess_flatten_edge (MTess, Tess, TessId, CTessIds, CTessQty, &FTess, TessE, &Flatten);

  neut_flatten_init_domstuff_fromver (&Flatten, Tess[TessId]);
  neut_flatten_init_domstuff_fromedge (&Flatten, Tess[TessId]);

  net_mtess_flatten_face (MTess, Tess, TessId, CTessIds, CTessQty, &FTess, &TessE, &Flatten);

  neut_flatten_init_domstuff_fromface (&Flatten);

  net_mtess_flatten_poly (MTess, Tess, TessId, CTessIds, CTessQty, &FTess, &TessE, &Flatten);
