...
void
net_mtess_flatten_face (struct MTESS MTess, struct TESS *Tess,
		  ...
		  struct FLATTEN *pFlatten)
{
  ...
  (*pTessE)[TessId].FaceFFaceOri = ut_alloc_1d_pint (Tess[TessId].FaceQty + 1);
  ...
  net_mtess_flatten_face_body (MTess, Tess, CTessIds, CTessQty, pFTess, pTessE, pFlatten);

  net_mtess_flatten_face_domface (MTess, Tess, TessId, pFTess, pTessE, pFlatten);
