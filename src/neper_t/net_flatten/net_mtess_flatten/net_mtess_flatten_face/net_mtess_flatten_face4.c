  ...
  for (i = (*pedgepos); i < edgeqty; i++)
  {
    ...
    for (j = 0; j < edgetessfaceqty[edgeinv][0]; j++)
    {
      ...
      for (k = 0; k < edgetessfaceqty[edgeinv][1]; k++)
      {
        ...
        if (ut_array_2d_int_linepos
            (*pfacepairlist, *pfacepairqty, 2, facepair) == -1)
        {
          (*pfacepairlist) =
            ut_realloc_2d_int_addline (*pfacepairlist, ++(*pfacepairqty), 2);
          ut_array_1d_int_memcpy (facepair, 2,
                                  (*pfacepairlist)[(*pfacepairqty) - 1]);
          (*pedgepos) = i;
          return 1;
        }
      }
...
void
net_mtess_flatten_face_domface_body_nextedge (struct FLATTEN Flatten,
                                              struct TESS DTess, int dface,
                                              struct TESS FTess,
                                              int *tessfaces, int fedge,
                                              int fver, int *edgesinv,
                                              int ***edgetessfacenb,
                                              int **edgetessfaceqty,
                                              int *pnedge, int *pnedgeori)
{
  ...
  status = ut_array_1d_int_rmelt (tmp, qty, fedge, 1);
  if (status != 1)
    abort ();
  qty--;
  ...
  for (j = 0; j < qty; j++)
  {
    ...
    for (k = 0; k < 2; k++)
      if (ut_array_1d_int_eltpos
          (edgetessfacenb[edgeinv][k], edgetessfaceqty[edgeinv][k],
           tessfaces[k]) != -1)
        status++;
