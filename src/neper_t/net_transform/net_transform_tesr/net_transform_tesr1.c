...
net_transform_tesr (struct IN_T In, struct TESR *pTesr)
{
  ...
  for (i = 0; i < PartQty; i++)
  {
    ...
    else if (!strncmp (parts[i], "tesrrenumber", 12))
    {
      ut_print_message (0, 2, "Renumbering from tesr...\n");
      net_transform_tesr_tesrrenumber (In, parts[i], pTesr);
    }
    else if (!strncmp (parts[i], "tesrtranslate", 12))
    {
      ut_print_message (0, 2, "Translating from tesr...\n");
      net_transform_tesr_tesrtranslate (parts[i], pTesr);
    }
    else if (!strncmp (parts[i], "tesrmerge", 9))
    {
      ut_print_message (0, 2, "Merging tesr...\n");
      net_transform_tesr_tesrmerge (parts[i], pTesr);
    }
    else if (!strncmp (parts[i], "addbuffer", 9))
    {
      ut_print_message (0, 2, "Adding buffer...\n");
      neut_tesr_addbuffer_string (pTesr, parts[i]);
    }
    ...
    else if (!strncmp (parts[i], "orirotate", 9))
    {
      ut_print_message (0, 2, "Rotating crystal orientations...\n");
      net_transform_tesr_orirotate (parts[i], pTesr);
    }
    else if (!strncmp (parts[i], "mergecell", 9))
    {
      ...
      for (j = 0; j < exprqty; j++)
      {
        qty = neut_tesr_cellexpr_merge (pTesr, exprs[j]);
