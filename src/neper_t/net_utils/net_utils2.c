...
net_tess_tesr_cell (struct TESS Tess, int cell, struct TESR *pTesr)
{
  ...
  for (k = (*pTesr).CellBBox[cell][2][0]; k <= (*pTesr).CellBBox[cell][2][1];
       k++)
    for (j = (*pTesr).CellBBox[cell][1][0];
         j <= (*pTesr).CellBBox[cell][1][1]; j++)
      for (i = (*pTesr).CellBBox[cell][0][0];
           i <= (*pTesr).CellBBox[cell][0][1]; i++)
      {
        ...
        if (neut_tess_point_incell (Tess, coo, cell) == 1)
        {
          ...
          if (pos[0] >= 1 && pos[0] <= (*pTesr).size[0] && pos[1] >= 1
              && pos[1] <= (*pTesr).size[1] && pos[2] >= 1
              && pos[2] <= (*pTesr).size[2])
