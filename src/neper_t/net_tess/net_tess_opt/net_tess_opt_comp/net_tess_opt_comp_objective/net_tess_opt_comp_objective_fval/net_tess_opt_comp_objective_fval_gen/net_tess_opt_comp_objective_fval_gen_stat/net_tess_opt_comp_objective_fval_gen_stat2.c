...
{
  ut_fct_set_numerical ((*pTOpt).curpdf + var, (*pTOpt).tarpdf[var].min,
                        (*pTOpt).tarpdf[var].max, (*pTOpt).tarpdf[var].size,
                        NULL);
  ut_fct_set_numerical ((*pTOpt).curcdf0 + var, (*pTOpt).tarpdf[var].min,
                        (*pTOpt).tarpdf[var].max, (*pTOpt).tarpdf[var].size,
                        NULL);
...
void
net_tess_opt_comp_objective_fval_gen_stat_smoothed_comp (struct TOPT *pTOpt,
                                                         int var)
{
  int i, j;
  double val, deltax;

  ut_fct_set_numerical ((*pTOpt).curcdf + var, (*pTOpt).tarcdf[var].min,
                        (*pTOpt).tarcdf[var].max, (*pTOpt).tarcdf[var].size,
                        NULL);

  ut_array_1d_zero ((*pTOpt).curcdf[var].y, (*pTOpt).curcdf[var].size);
...
void
net_tess_opt_comp_objective_fval_gen_stat_smoothed_update (struct TOPT *pTOpt,
                                                           int var)
{
  int i, cell;
  ...
  for (i = 0; i < (*pTOpt).cellchangedqty; i++)
  {
    ...
    net_tess_opt_comp_objective_fval_gen_stat_smoothed_update_cell (pTOpt,
                                                                    var,
                                                                    cell);
  }
...
void
net_tess_opt_comp_objective_fval_gen_stat_smoothed_gen_legacy (struct TOPT
                                                               *pTOpt,
                                                               int var)
{
...
void
net_tess_opt_comp_objective_fval_gen_stat_smoothed_update_legacy (struct TOPT
                                                                  *pTOpt,
                                                                  int var)
{
...
void
net_tess_opt_comp_objective_fval_gen_stat_unsmoothed_comp (struct TOPT *pTOpt,
                                                           int var)
{
  int i, j;

  ut_array_1d_zero ((*pTOpt).curcdf0[var].y, (*pTOpt).curcdf0[var].size);
  for (j = 1; j <= (*pTOpt).CellQty; j++)
    if ((*pTOpt).curcellpenalty[j] == 0)
...
void
net_tess_opt_comp_objective_fval_gen_stat_evaluate (struct TOPT *pTOpt,
                                                    int var)
{
