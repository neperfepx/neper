			...
			struct SEEDSET *SSet, struct TOPT *pTOpt)
{
  ...
  if ((*pTOpt).tarqty > 0)
    ut_print_message (0, 3, "Setting seeds... ");

  net_tess_opt_init_sset_general (In, MTess, Tess, dtess, dcell, SSet,
      &(*pTOpt).SSet, (*pTOpt).CellQty, pTOpt);

  net_tess_opt_init_sset_pre (In, level, MTess, Tess, dtess, dcell, &var, &pos, &weightexpr,
			      &cooexpr, pTOpt);

  net_tess_opt_init_sset_weight (MTess, Tess, dtess, dcell, *pTOpt, var, pos, weightexpr, rad);

  net_tess_opt_init_sset_multiseed (In, level, *pTOpt, var, rad, qty);

  net_tess_opt_init_sset_coo (MTess, Tess, dtess, dcell, pTOpt, var, pos, cooexpr, rad, qty);

  net_tess_opt_init_sset_ori (In, level, MTess, Tess, dtess, dcell, SSet, &(*pTOpt).SSet);

  net_tess_opt_init_sset_post (In, level, Tess[dtess], dcell,
			       (*pTOpt).SSet, pTOpt);
