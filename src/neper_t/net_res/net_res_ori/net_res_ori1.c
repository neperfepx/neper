...
net_res_ori (struct IN_T In, struct SEEDSET SSet)
{
  ...
  if (strcmp (format, "plain") == 0)
  {
    if (strcmp (des, "e") == 0)
    {
      ...
      {
	ol_q_e (SSet.q[i], e);
	if (strcmp (crysym, "triclinic") != 0)
	  ol_e_ecrysym (e, crysym, e);
	...
	if (strcmp (crysym, "triclinic") != 0)
	  ol_rtheta_rthetacrysym (r, theta, crysym, r, &theta);
    ...
    else if (strcmp (des, "q") == 0)
    {
      ...
      {
	if (strcmp (crysym, "triclinic") != 0)
	  ol_R_Rcrysym (SSet.q[i], crysym, SSet.q[i]);
