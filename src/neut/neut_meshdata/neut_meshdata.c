...
void
neut_meshdata_mesh2slice (struct MESHDATA *MeshData, struct MESH SMesh,
                          int *elt_newold, struct MESHDATA **pSMeshData)
{
  int i, j, size;
  ...
  if (MeshData[3].ColDataType)
  {
    ...
    if (size > 0)
    {
      (*pSMeshData)[2].ColData =
        ut_alloc_2d ((*pSMeshData)[2].EltQty + 1, size);
