                                    ...
                                    int k, int connec, int interior)
{
  ...
  if ((Tesr.VoxCell[i - 1][j][k] == cell || Tesr.VoxCell[i - 1][j][k] <= min)
      && (Tesr.VoxCell[i + 1][j][k] == cell
          || Tesr.VoxCell[i + 1][j][k] <= min)
      && (Tesr.VoxCell[i][j - 1][k] == cell
          || Tesr.VoxCell[i][j - 1][k] <= min)
      && (Tesr.VoxCell[i][j + 1][k] == cell
          || Tesr.VoxCell[i][j + 1][k] <= min)
      && (Tesr.VoxCell[i][j][k - 1] == cell
          || Tesr.VoxCell[i][j][k - 1] <= min)
      && (Tesr.VoxCell[i][j][k + 1] == cell
          || Tesr.VoxCell[i][j][k + 1] <= min))
    res = 0;
                                    ...
                                    int connec, int interior)
{
  ...
  if ((Tesr.VoxCell[i - 1][j][1] == cell || Tesr.VoxCell[i - 1][j][1] <= min)
      && (Tesr.VoxCell[i + 1][j][1] == cell
          || Tesr.VoxCell[i + 1][j][1] <= min)
      && (Tesr.VoxCell[i][j - 1][1] == cell
          || Tesr.VoxCell[i][j - 1][1] <= min)
      && (Tesr.VoxCell[i][j + 1][1] == cell
          || Tesr.VoxCell[i][j + 1][1] <= min))
    res = 0;
