...
void
neut_mesh_str (int dim, int *size, struct NODES *pNodes, struct MESH
	       *pMesh, struct NSET *pNSet)
{
  if (dim == 3)
...
void
neut_mesh_str_3d (int *size, struct NODES *pNodes, struct MESH
...
void
neut_mesh_str_2d (int *size, struct NODES *pNodes, struct MESH
		  *pMesh, struct NSET *pNSet1D)
{
  ...
  if (pNSet1D)
  {
	...
	for (i = 1; i <= (*pNodes).NodeQty; i++)
      ...
      ut_array_1d_int_memcpy ((*pNSet1D).nodes[edge], tmp[0], tmp + 1);
    }
  }
...
void
neut_mesh_str_1d (int *size, struct NODES *pNodes, struct MESH
		  *pMesh, struct NSET *pNSet1D)
{
  ...
  if (pNSet1D)
  {
	...
	for (i = 1; i <= (*pNodes).NodeQty; i++)
      ...
      ut_array_1d_int_memcpy ((*pNSet1D).nodes[ver], tmp[0], tmp + 1);
    }
  }
