...
/* RAM */
void
ReadEltsProp (FILE * msh, char *mode, struct MESH *pMesh, int **pelt_nbs,
              int MaxEltQty)
{
  ...
  else
  {
    while ((*pMesh).EltQty < MaxEltQty)
    {
      ...
      for (i = 1; i <= eltqty; i++)
        (*pMesh).EltNodes =
          ut_realloc_2d_int_addline ((*pMesh).EltNodes,
                                     (*pMesh).EltQty + i + 1, eltnodeqty);

      (*pMesh).EltElset =
        ut_realloc_1d_int ((*pMesh).EltElset, (*pMesh).EltQty + eltqty + 1);

      for (i = 1; i <= eltqty; i++)
      {
        id = ++(*pMesh).EltQty;
        if (fread (pelt_nbs ? (*pelt_nbs) + id : &tmp, sizeof (int), 1, msh)
            != 1)
          abort ();
        ...
        for (j = 0; j < tagqty - 1; j++)
          if (fread (&tmp, sizeof (int), 1, msh) != 1)
            abort ();
        if (fread ((*pMesh).EltNodes[id], sizeof (int), eltnodeqty, msh) !=
