...
/* Ctopoyright (C) 2003-2019, Romain Quey. */
			    ...
			    int dface, int **pedges, int *pedgeqty)
{
  ...
  ut_array_1d_int_memcpy (*pedges,
			  Flatten.DomFaceTessEdgeQty[dface],
			  Flatten.DomFaceTessEdgeNb[dface]);

  return;
}

void
neut_flatten_domface_ver_edges (struct FLATTEN Flatten, struct TESS
				DTess, int dface, struct TESS FTess,
				int ver, int **pedges, int *pedgeqty)
{
  ...
  for (i = 0; i < FTess.VerEdgeQty[ver]; i++)
  {
    ...
    else if (Flatten.EdgeDom[edge][0] == 1)
    {
      ...
      // to the domain face.
      pos = ut_array_1d_int_eltpos (DTess.EdgeFaceNb[dedge],
