...
void
neut_boundary_bound_nodes_2d (struct MESH Mesh, struct BOUNDARY Bound, int id,
                              int side, int **pnodes, int *pnodeqty)
{
  ...
  neut_elt_facet_nodes (Mesh.EltType, Mesh.Dimension, Mesh.EltOrder,
                        Mesh.EltNodes[elt], facet, 1, surfnodes);
  ...
  if (Bound.BoundEltQty[id] > 1)
  {
    ...
    neut_elt_facet_nodes (Mesh.EltType, Mesh.Dimension, Mesh.EltOrder,
                          Mesh.EltNodes[nextelt], nextfacet, 1,
                          nextsurfnodes);
...
void
neut_boundary_bound_nodes_3d (struct MESH Mesh, struct BOUNDARY Bound, int id,
                              int side, int **pnodes, int *pnodeqty)
{
  int i, j, elt, facet;
